...
Murphet (β‑likelihood, MAP, Optuna‑tuned)  vs  Prophet
...
def mur_cfg(trial):
    # 1️⃣  seasonal structure
    periods, harms = [12.0], [trial.suggest_int("harm_year", 1, 4)]
    if trial.suggest_categorical("add_half", [0,1]):
        periods.append(6.0);  harms.append(trial.suggest_int("harm_half", 1, 4))
    if trial.suggest_categorical("add_qtr",  [0,1]):
        periods.append(3.0);  harms.append(trial.suggest_int("harm_qtr",  1, 4))

    # 2️⃣  trend / prior hyper‑params
...
ax1.plot(df["ds"], df["y"], "k.-", lw=1, label="Actual")
ax1.plot(train_ho["ds"], mur_fit.predict(train_ho["t"]),
         lw=1.6, color="royalblue", label="Murphet fit")
ax1.plot(train_ho["ds"], prop_fit.predict(train_ho[["ds"]])["yhat"],
         lw=1.6, color="darkorange", label="Prophet fit")
ax1.plot(test_ho["ds"], mur_pred, "o--", color="royalblue", label="Murphet fcst")
ax1.plot(test_ho["ds"], prop_pred,"o--",color="darkorange",label="Prophet fcst")
ax1.fill_between(test_ho["ds"], prop_low, prop_upp, color="darkorange",
                 alpha=.25,label="Prophet 80 % CI")
ax1.axvline(test_ho["ds"].iloc[0],ls="dotted",color="gray")
ax1.set_title(f"Retail I/S ratio – {HOLD_OUT_MO}‑month hold‑out")
ax1.set_ylabel("1 / Inventories‑to‑Sales"); ax1.legend(); fig1.tight_layout()
